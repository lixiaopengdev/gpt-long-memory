...
const authReducer = async (state = { authData: null }, action) => {
  switch (action.type) {
    ...
    case actionType.CLEAR_CACHE:
      ...
      if(data?.error) {
        return alert("27 There was an error while processing your request. Please try to refresh the page, if the error persists clear the chat and/or the cache. Finally if the erorr is still present try again later.")
      }
